<template>
  <div class="dl-overlay">
    <div class="deleteModal">
      <h1 class="text-center mb-4">Delete {{ name }}</h1>
      <p class="text-danger fw-bold mb-3">
        Are you sure you want to delete this restaurant ...
      </p>
      <div class="d-flex justify-content-between">
        <button class="btn btn-secondary" @click="emitClose">No</button>
        <button class="btn btn-danger" @click="deleteRestaurant">Yes</button>
      </div>
    </div>
  </div>
  <teleport to="body">
...
export default {
  ...
  async mounted() {
    let result = await axios.get(
      `http://localhost:3000/rests?id=${this.id}&userId=${this.userId}`
    );
  ...
  methods: {
    ...
    async deleteRestaurant() {
      ...
      // now delete the items , categories and then the restaurant
      let flag = true;
      ...
      let result = await axios.delete(`http://localhost:3000/rests/${this.id}`);
      if (result.status == 200) {
        this.theme = "success";
        this.notify = `Deleted ${this.name} Successfully`;
        this.showNotification = true;
        setTimeout(() => {
          this.showNotification = false;
          location.reload();
